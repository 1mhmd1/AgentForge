...
OUTPUT_DIR = "output"
DOMAIN = "web_research"
GOAL = "Summarize top 5 AI trends for 2026"
COMPLEXITY = "simple"
...
# ── Research Content ─────────────────────────────────────
RESEARCH_CONTENT = "<html><head><style>.search-container{width:100%;padding:24px}.search-container input[type=\"text\"]{width:100%;padding:12px 24px}.search-container button{background-color:#4CAF50;color:#fff;padding:12px 24px}.search-container button:hover{background-color:#3e8e41}</style></head><body><div class=\"search-container\"><input type=\"text\" placeholder=\"Search AI trends for 2026\"><button>Search</button></div><script>function searchQuery() { var query = document.querySelector(\"input[type='text']\").value; var searchQuery = { query: query }; return searchQuery; }</script><script>var searchQuery = searchQuery(); console.log(searchQuery);</script><script>var trends = [{\"name\": \"Explainable AI"
