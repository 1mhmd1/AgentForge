...
response = requests.get('https://newsapi.org/v2/everything', params={'q': 'cloud architecture', 'sortBy': 'publishedAt', 'apiKey': 'demo'})
...
for article in search_results:
    summary_results.append(article['title'] + ': ' + article['description'])
...
# Organize findings into a structured report
report = {}
report['Introduction'] = 'Cloud architecture best practices'
report['Methodology'] = 'Search, scrape, and summarize credible sources'
report['Findings'] = []

# Append findings from previous steps
previous_steps = {'step_1': {'summary': 'Fetched recent articles about cloud architecture using NewsAPI'}, 'step_2': {'summary': 'Gathered relevant information from credible sources by searching for cloud architecture articles and extracting their titles and descriptions'}}
for step_id, step_result in previous_steps.items():
    report['Findings'].append(step_result['summary'])
...
# Summarize key takeaways and recommendations
summary = 'Based on the research, the key takeaways are: cloud architecture best practices include scalability, security, and cost-effectiveness. Recommendations include using cloud-native services, implementing monitoring and logging, and ensuring data backup and recovery.'
