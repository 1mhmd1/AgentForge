...
def validate_audit(audit: dict) -> dict:
    ...
    # agents_executed may be a count (legacy int) or the actual list of agent
    # ids (current builder writes a list per BUG-E fix). Accept both shapes.
    agents_count: int | None = None
    if isinstance(agents_executed, list):
        if not all(isinstance(item, str) and item.strip() for item in agents_executed):
            errors.append("agents_executed_invalid_list_items")
        else:
            agents_count = len(agents_executed)
    elif _is_non_negative_int(agents_executed):
        agents_count = agents_executed
    else:
