  ...
  return (
    <div data-responsive-root style={s.root}>
      ...
      <div style={s.table}>
        ...
        {loading && <EmptyRow>Loading runs… <SpinnerIcon size={14} /></EmptyRow>}
...
function EmptyRow({ children, danger }: { children: React.ReactNode; danger?: boolean }) {
  return (
    <div style={{ padding: '24px 24px', fontFamily: 'Inter, sans-serif', fontSize: 13, color: danger ? '#FCA5A5' : '#94A3B8', display: 'flex', alignItems: 'center', gap: 8 }}>
      {children}
    </div>
  );
}

function statusLabel(status: RunStatus): { bg: string; color: string; border: string; label: string; running: boolean } {
  switch (status) {
    case 'COMPLETED': return { bg: 'rgba(34,197,94,0.12)', color: '#22C55E', border: 'rgba(34,197,94,0.25)', label: 'Success', running: false };
    case 'FAILED': return { bg: 'rgba(239,68,68,0.12)', color: '#EF4444', border: 'rgba(239,68,68,0.25)', label: 'Failed', running: false };
    case 'INTERRUPTED': return { bg: 'rgba(239,68,68,0.12)', color: '#EF4444', border: 'rgba(239,68,68,0.25)', label: 'Interrupted', running: false };
    case 'CANCELLED': return { bg: 'rgba(148,163,184,0.12)', color: '#94A3B8', border: 'rgba(148,163,184,0.25)', label: 'Cancelled', running: false };
    default: return { bg: 'rgba(59,130,246,0.12)', color: '#3B82F6', border: 'rgba(59,130,246,0.25)', label: status.charAt(0) + status.slice(1).toLowerCase(), running: true };
  }
}
...
function RunRow({ run, index, onClick }: { run: RunSummary; index: number; onClick: () => void }) {
  const [hover, setHover] = useState(false);
  const b = statusLabel(run.status);
  const agent = agentByDomain(run.domain);
  const duration = typeof run.buildDurationSec === 'number' ? `${run.buildDurationSec.toFixed(1)}s` : '—';
  return (
    <div onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 24px', borderBottom: '1px solid rgba(26,39,64,0.3)', background: hover ? 'rgba(124,58,237,0.06)' : 'transparent', position: 'relative', cursor: 'pointer', transition: 'background 150ms ease', animation: `fadeUp 300ms ease ${index * 40}ms both`, fontFamily: 'Inter, sans-serif' }}>
      ...
      <span style={{ flex: '0 0 130px', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: '#A78BFA' }}>{run.id.slice(0, 12)}</span>
      <span style={{ flex: 1, fontSize: 14, color: '#94A3B8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{run.prompt}</span>
      <span style={{ flex: '0 0 140px', fontSize: 13, color: '#94A3B8' }}>{agent.name}</span>
      <span style={{ flex: '0 0 110px' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 100, background: b.bg, color: b.color, border: `1px solid ${b.border}` }}>
          {b.running && <span style={{ width: 5, height: 5, borderRadius: '50%', background: b.color, animation: 'pulse-status 1.2s infinite' }} />}
          {b.label}
        </span>
      </span>
      <span style={{ flex: '0 0 90px', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: '#475569' }}>{duration}</span>
      <span style={{ flex: '0 0 90px', fontSize: 12, color: '#475569', textAlign: 'right' }}>{formatWhen(run.createdAt)}</span>
    </div>
  );
}
